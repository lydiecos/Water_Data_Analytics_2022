...
## Data wrangling
```{r}
# Check out available LAGOS datasets
...
secchi.summary <- LAGOScombined %>%
  filter(sampledate > "2001-12-31") %>%
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            secchi.min = min(secchi),
...
```{r}
# 1a. Filter datasets
maine.state <- states.subset %>%
  filter(ID == "maine")

maine.secchi <- LAGOScombined %>%
  filter(state == "ME")
  
# 1b. Create summary
maine.summary <- maine.secchi %>%
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            secchi.min = min(secchi),
            area = mean(lake_area_ha),
...
```{r}
plot_grid(plot.secchi.area, plot.secchi.depth)
```
