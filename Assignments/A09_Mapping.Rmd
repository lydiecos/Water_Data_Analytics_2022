...
# create sf object to hold Indiana
states <- st_as_sf(map(database = "state", plot = FALSE, fill = TRUE, col = "white"))
states.subset <- filter(states, ID == "indiana")

# combine datasets and filter to Indiana
...
# graph CHLA levels for each lake
...
subset_file <- tempfile(fileext = ".gpkg") # required step to run the subset_nhdplus function
...
gages <- get_nwis(AOI = catchment) # AOI = Area of Interest
class(gages)

# find gages only within watershed
gages <- st_intersection(gages, catchment)

# ready made plot with basemap
plot_nhdplus("USGS-02085070", streamorder = 3)
...
> The upstream area is 367.7 square kilometers. Water tends to flow from higher elevations to lower elevations, ending up in areas that are flatter, which will have higher stream order because of the water accumulation. 123 flowpaths of stream order 1, 45 of stream order 2, 19 of stream order 3, and 28 of stream order 4.

5. Are there any other gage sites in the upstream catchment? If so, where are they?
> Yes, there are two other gages, one in Hillsborough and one at Cole Mill Road.
