...
> aggregation: Take a certain time step and go to a broader time step to smooth across gaps in data. May assume that there is no temporal autocorrelation (but test will run regardless).

> subsampling: Use part of the data to build a model and use a different part to test the model. Or, looking at a different time frequency (e.g., have daily data but only look at datapoints once every 14 days)
...
NeuseFlow_monthly <- NeuseFlow %>%
  mutate(Year = year(Date),
         Month = month(Date)) %>%
  group_by(Year, Month) %>%
  summarise(Discharge_acftmo = sum(Discharge)*1.98347) # change units
...
ggplot(subset(NeuseCond_monthly, !is.na(Month)), aes(x = as.factor(Month), y = Conductivity)) +
  geom_boxplot() +
  labs(x = "Month", y = expression("Conductivity ("*mu*"/cm)")) +
  theme(axis.text.x = element_blank())

Flowplot <-
ggplot(NeuseFlow_monthly, aes(x = as.factor(Month), y = Discharge_acftmo)) +
  geom_boxplot() +
  labs(x = "Month", y = "Discharge (acre ft/mo)")
...
# Inspect results
NeuseCond_trend
# S-score shows the direction and strength of the trend over time
# For example, the S for January is the combination of Januarys across all years. An S-score close to zero means no change over time.
...
> The Neuse River is showing signs of salinization over time (high positive z-score, tiny p-value). This trend is seen in the first half of the year, late winter through early summer.
...
> The Neuse River is showing an increase in discharge over time, though not as strong of an effect (p = 0.0027). Increase is occuring during the fall months. The monthly effects are different from the salinization - higher flow will dilute salinity. 
...
# The data are stationary (not changing) over time
...
# Given one month of data, how auto-correlated will the data be with following months?
# Point is that autocorrelation decays over time and that we can see some seasonality
pacf(NeuseFlow_timeseries)
# Decays faster and less seasonality
