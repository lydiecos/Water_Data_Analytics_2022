...
- Excessive nutrients from agriculture -> algal blooms, eutrophication
- Sediment loading -> higher turbidity, lower clarity, lower production
- Thermal pollution -> reduces habitat
- Chemicals (metals, toxins)
- Bacteria, fungae, viruses
We'll be focusing on eutrophication this week
...
options(scipen = 10) # sets number of digits to report
...
# if colnames are different: by = c("state_zoneid" = "ZoneID")
...
* *Secchi disk transparency*, a measure of water clarity. Pros: simple and cheap. Cons: may yield a high TSI in highly colored lakes and in lakes where particulate matter is comprised of non-algal material. White & Black disk that is lowered into the water until its colors can no longer be distinguished.  
...
```{r}
dim(LAGOSnutrient) # huge database
class(LAGOSnutrient$sampledate) # date is already a date!

LAGOStrophic <- LAGOSnutrient %>%
  left_join(., LAGOSlocations, by = "lagoslakeid") %>% #skip naming first df bc of pipe
...
> Higher, larger range, and much larger sample size of Chla in the warm months. Higher median depth of clarity in the winter, declines with increased chlorophyll in the summer. More consistent phosphorous levels.
...
LAGOStrophic <- LAGOStrophic %>%
  filter(samplemonth %in% c(5:9)) %>% #filter for growing season
...
# ^logical statement to classify trophic levels by chla score
...
# We will be using this color palette later, need colormap package:
...
write.csv(LAGOStrophic, file = "./Data/Processed/LAGOStrophic.csv", row.names = FALSE) # Stopped here on Feb 15
```
...
> Chl is higher than secchi -> happens when all/most of clarity reduction is coming from chlorophyll, such as with an algal bloom. If secchi is higher than chl, then there is turbidity occurring that is not part of primary productivity - suspended particulate matter reducing clarity. 

> Chl is higher than tp -> primary production is closely related to phosphorus limitation. Tp is higher than Chl -> something else besides phosphorus is limiting primary production.
