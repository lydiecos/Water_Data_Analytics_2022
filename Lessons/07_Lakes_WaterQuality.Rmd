...
LAGOSMN <- LAGOSjoined %>%
  ...
  filter(month %in% c(6:9) & year > 1989 & state_name == "Minnesota") %>%
  filter(hu4 %in% c("0401", "0701", "0702", "0703", "0704", "0902"))

ggplot(LAGOSMN, aes(x=chla, y=tp, color=hu4)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color="black") +
  theme_bw() +
  facet_wrap(vars(hu4)) +
  labs(x="chlorophyll", y="Total Phosphorus", color="HUC 4")

lmMN <- lm(data=LAGOSMN, log10(tp) ~ log10(chla) * hu4)
summary(lmMN)
par(mfrow = c(2,2))
plot(lmMN)
...
# customize: hide duplicates and diagonal
corrplot(LAGOScorr, type = "upper", diag = FALSE, tl.cex = 0.8, tl.col = "black") 

```

Conclusions:
- doc = dissolved organic carbon -> lower concentration with deeper lake
- positive relationship between area, depth, percent forest, slope
- pct row crops is positively associated with chla