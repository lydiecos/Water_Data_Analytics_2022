...
> Very peaky and seasonal. Slight linear trend upward over time but not a great candidate for linear trend analysis. Could remove seasonality to detect trend. 
...
```{r}
# Create time series
NeuseCond_ts <- ts(NeuseCond_monthly$Conductivity, frequency = 12)

# Decompose time series
NeuseCond_Decomposed <- stl(NeuseCond_ts, s.window = "periodic")

# Plot time series
plot(NeuseCond_Decomposed)
# Yes, there is seasonality but it does not substantially impact the trend. The seasonal component is an order of magnitude smaller than the overall variability. 
